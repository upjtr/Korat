...
function Silk() {
  ...
  const getRice = async () => {
    ...
    if (res.data) {
      const d = res.data.filter((el) => el.product_type === "silk");
      setdata(d);
    }
  };
  useEffect(() => {
  ...
  return (
    <div>
      <div className="relative min-h-[570px] h-[70vh] max-h-[800px] w-full">
        <img
          src={bgSilk}
          className="h-full w-full object-cover object-center image-silk"
        />
        <div className="title-rice">
          <h1 className="pt-[2rem]">ผ้าไหม</h1>
        </div>
      </div>
      ...
      <div className="my-[5rem]">
        <Container>
          <h1 className="banner-title text-[38px]">
            ผลิตภัณฑ์จากผ้าไหม {data?.length} รายการ
          </h1>
          <div className="line" />
